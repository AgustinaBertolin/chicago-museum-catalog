...
export const ArtworkScreen = ({navigation, route}: ArtworkScreenRouteProp) => {
  ...
  const fullScreen = () =>{
    navigation.navigate('FullscreenImage', {uri: `https://www.artic.edu/iiif/2/${image_id}/full/250,/0/default.jpg`, name: artwork?.name ?? ''})
  }

  return (
    <View style={styles.container}>
      <Pressable style={styles.image} onPress={fullScreen}>
        <ImageBackground style={styles.image} source={{uri: `https://www.artic.edu/iiif/2/${image_id}/full/250,/0/default.jpg`}}/>
      </Pressable>
      <View style={styles.dataContainer}>
        ...
        <ScrollView>
          ...
          <Text style={styles.description}><Text style={styles.bold}>Artist: </Text>{convert(artwork?.artist.display, {wordwrap: 130})}</Text>
          <Text style={styles.description}><Text style={styles.bold}>Origin: </Text>{artwork?.origin}</Text>
          {artwork?.techniques.length !== 0 && <Text style={styles.description}><Text style={styles.bold}>Techniques: </Text>{artwork?.techniques.join(', ')}</Text>}
          {artwork?.styles.length !== 0 && <Text style={styles.description}><Text style={styles.bold}>Styles: </Text>{artwork?.styles.join(', ')}</Text>}
          {artwork?.categories.length !== 0 && <Text style={styles.description}><Text style={styles.bold}>Categories: </Text>{artwork?.categories.join(', ')}</Text>}
          {artwork?.copyright && <Text style={styles.description}><Text style={styles.bold}>Copyright: </Text>{artwork?.copyright}</Text>}
        </ScrollView>
      </View>
    </View>
  )
...
type ArtworkScreenRouteProp = NativeStackScreenProps<
  RootStackParamList,
  'Artwork'
>;